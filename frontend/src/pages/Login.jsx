...
const Login = () => {
  ...
  const { token, setToken, navigate, bakendUrl } = useContext(ShopContext)
    ...
    try {
      if (currentState === 'Sign Up') {
        ...
        if (response.data.success) {
          ...
          localStorage.setItem('token', response.dated.token)
        } else {
          toast.error(response.data.message)
        }
      } else {
        const response = await axios.post(bakendUrl + '/api/user/login', { email, password })
        if (response.data.success) {
          setToken(response.data.token)
